...
public class FormatString {
    ...
    public FormatString(String defaultTranslation, Map<String, String> additionalTranslations) {
        this(escapeStringForMessageFormat(defaultTranslation, DEFAULT_LOCALE),
                additionalTranslations == null ? null : additionalTranslations.entrySet()
                        .stream()
    ...
    String translateTo(Locale locale, Object... args) {
        ...
        try {
            ...
            throw new FormatException("Failed to format string '%s' with locale '%s' and %d arguments of types (%s)".formatted(
                    formatString.toPattern(),
                    locale,
                    args.length,
                    Arrays.stream(args).map(a -> a.getClass().getTypeName()).collect(Collectors.joining(","))), e);
        }
    }
