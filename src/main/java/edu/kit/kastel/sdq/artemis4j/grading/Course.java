...
public class Course extends ArtemisConnectionHolder {
    ...
    public Course(CourseDTO dto, ArtemisConnection connection) {
        ...
        this.exams =
                new LazyNetworkValue<>(() -> ExamDTO.fetchAll(connection.getClient(), dto.id()).stream().map(examDTO -> new Exam(examDTO, this)).toList());
    }
    ...
    public Exam getExamById(long id) throws ArtemisNetworkException {
        return this.exams.get().stream().filter(e -> e.getId() == id).findAny().orElseThrow(() -> new IllegalArgumentException("No exam with id " + id + " found"));
    }
}
